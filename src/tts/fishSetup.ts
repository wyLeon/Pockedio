...
export function detectFishTtsInstall(config: PockedioConfig, pockedioHome: string, cwd = process.cwd()): FishSetupDetection {
  ...
  const missing = [
    ...
    modelDir ? undefined : "Fish S2 Pro model directory",
...
export function buildFishSetupInstallCommands(cwd = process.cwd()): FishSetupCommand[] {
  return [
    ...
    },
    {
      ...
      command: "uv",
      args: ["venv", path.join(cwd, ".cache", "mlx-speech-venv"), "--python", "3.13"]
    },
    {
      label: "Install mlx-speech",
      command: path.join(cwd, ".cache", "mlx-speech-venv", "bin", "python"),
      args: ["-m", "pip", "install", "-e", path.join(cwd, ".cache", "mlx-speech")]
    },
    {
...
function firstExistingDirectory(candidates: Array<string | undefined>, cwd: string): string | undefined {
  return candidates.map((candidate) => resolveCandidate(candidate, cwd)).find((candidate): candidate is string => Boolean(candidate && fs.existsSync(candidate) && fs.statSync(candidate).isDirectory()));
}
