...
const PLAYBACK_TRANSITION_SUPERSEDED = "__pockedio_playback_transition_superseded__";
...
type PendingSongOrStationSelection = {
  originalRequest: string;
  query: string;
  candidates: PendingSingleTrackCandidate[];
};

type QueueToneChangeChoice = "reshape" | "new_station" | "keep";

type PendingQueueToneChangeSelection = {
...
export type InteractivePlaybackState = {
  ...
  pendingDjVersionRequest?: string;
...
function readTasteMarkdown(config: PockedioConfig | undefined): string {
  if (!config || !fs.existsSync(config.paths.taste)) {
    return "";
  }
  return fs.readFileSync(config.paths.taste, "utf8").trim();
}
...
export async function runSessionTurn(input: SessionTurnInput): Promise<SessionTurnResult> {
  ...
  try {
    ...
    if (input.playbackState?.pendingDjVersionRequest && isPendingStationConfirmation(input.input)) {
      const pendingRequest = input.playbackState.pendingDjVersionRequest;
      input.playbackState.pendingDjVersionRequest = undefined;
      ...
      const prepared = await prepareCurrentStationDjProgram({
        config,
        sessionId,
        store,
        llm,
    ...
    if (input.playbackState?.pendingDjVersionRequest && isPendingStationDecline(userText)) {
      input.playbackState.pendingDjVersionRequest = undefined;
      const response = "Keeping the current station.";
      store.addMessage(sessionId, "pockedio", response);
      writeOutput(response);
      return { sessionId, intent: { type: "pending_station_decline", confidence: "high" }, response, shouldExit: false };
    }
    if (input.playbackState?.pendingDjVersionRequest) {
      input.playbackState.pendingDjVersionRequest = undefined;
    }
    ...
    if (!userText && !input.playbackState?.pendingStationRequest && !input.playbackState?.pendingQueueToneChangeSelection) {
    ...
    let intent = pendingSingleTrackCandidate
      ...
      : pendingSongOrStationChoice === "song"
        ? { type: "single_track_playback" as const, confidence: "high" as const }
      : pendingSongOrStationChoice === "station"
        ? { type: "playback_request" as const, confidence: "high" as const }
      : pendingSongOrStationCancel
        ? { type: "conversation" as const, confidence: "high" as const }
      : pendingQueueToneChangeChoice
      ...
      : isActiveStationChangeRequest(userText, input.playbackState)
        ? { type: "feedback_change_vibe" as const, confidence: "high" as const }
    ...
    const playbackStatusWithKnowledgeQuestion = intent.type === "playback_status"
      && (isCurrentTrackQuestion(userText) || isMusicKnowledgeQuestion(userText));
    if (groundedPlaybackKnowledgeQuestion
      && !hasExplicitPlaybackCommand(userText)
      && (!isProtectedOperationalIntent(intent.type) || isCurrentArtistBiographicalFollowupQuestion(userText) || playbackStatusWithKnowledgeQuestion)) {
      intent = { type: "conversation", confidence: "high" };
    }
    if (input.playbackState?.currentTrackId && isPositiveCurrentArtistPreference(userText)) {
      intent = { type: "conversation", confidence: "high" };
    }
    if (isActiveStationChangeRequest(userText, input.playbackState)) {
      intent = { type: "feedback_change_vibe", confidence: "high" };
    }
    ...
    if (pendingSongOrStationChoice === "song" && input.playbackState?.pendingSongOrStationSelection) {
      const selection = input.playbackState.pendingSongOrStationSelection;
      input.playbackState.pendingSongOrStationSelection = undefined;
      input.playbackState.pendingStationRequest = undefined;
      input.playbackState.pendingStationOriginalRequest = undefined;
      input.playbackState.pendingStationNeedsChoice = false;
      if (selection.candidates.length > 1) {
        ...
        const response = formatSingleTrackChoices(input.playbackState.pendingSingleTrackSelection);
        store.addMessage(sessionId, "pockedio", response);
        writeOutput(response);
        return { sessionId, intent, response, shouldExit: false };
      }
    ...
    if (pendingSongOrStationChoice === "station" && input.playbackState?.pendingSongOrStationSelection) {
      const selection = input.playbackState.pendingSongOrStationSelection;
      input.playbackState.pendingSongOrStationSelection = undefined;
      input.playbackState.pendingSingleTrackSelection = undefined;
      const playback = await handlePlaybackRequest({
        ...
        provider,
        llm,
        requestText: `play something for ${selection.query}`,
    ...
    if (pendingQueueToneChangeChoice && input.playbackState?.pendingQueueToneChangeSelection) {
      const selection = input.playbackState.pendingQueueToneChangeSelection;
      input.playbackState.pendingQueueToneChangeSelection = undefined;
      if (pendingQueueToneChangeChoice === "keep") {
        const response = "Keeping the current queue.";
        store.addMessage(sessionId, "pockedio", response);
        writeOutput(response);
        return { sessionId, intent, response, shouldExit: false };
      }
      if (pendingQueueToneChangeChoice === "new_station") {
        const playback = await handlePlaybackRequest({
          config,
          sessionId,
          store,
          provider,
          llm,
          requestText: selection.replacementRequest,
          intentType: "playback_request",
          buildContext: input.buildContext,
          writeStatus,
          playUrl,
        ...
        store.addMessage(sessionId, "pockedio", playback.response);
        writeOutput(playback.response);
        return { sessionId, intent: { type: "playback_request", confidence: "high" }, response: playback.response, shouldExit: false, station: playback.station };
      }

      const currentTrack = getCurrentPlaybackTrack(input.playbackState);
      const response = currentTrack
        ? await withStatus(writeStatus, "Reshaping queue...", () => applyRemainingQueueReshapeForFeedback({
            config,
            sessionId,
            store,
            provider,
            ...
            feedbackText: selection.feedbackText,
            playbackState: input.playbackState!,
            currentTrack,
            signal
          }), signal)
    ...
    if (intent.type === "queue_position_playback" && queuePositionIndex !== undefined) {
      const response = await playQueuedTrackAt(input.playbackState, config, store, provider, queuePositionIndex, input.playbackState?.startUrlPlayback ?? startUrlPlayback);
      store.addMessage(sessionId, "pockedio", response);
      writeOutput(response);
      return { sessionId, intent, response, shouldExit: false };
    }
    ...
    if (intent.type === "previous") {
      const response = await retreatPlayback(input.playbackState, config, store, provider, input.playbackState?.startUrlPlayback ?? startUrlPlayback);
      store.addMessage(sessionId, "pockedio", response);
      writeOutput(response);
      return { sessionId, intent, response, shouldExit: false };
    }
    ...
    if (intent.type === "replay") {
      const response = await replayCurrentTrack(input.playbackState, config, store, provider, input.playbackState?.startUrlPlayback ?? startUrlPlayback);
      store.addMessage(sessionId, "pockedio", response);
      writeOutput(response);
      return { sessionId, intent, response, shouldExit: false };
    }
    ...
    if (isFeedbackIntent(intent.type)) {
      ...
      const response = isQueueReshapeFeedback(action) && input.playbackState?.station && currentTrack
        ...
        ? await advancePlayback(input.playbackState, config, store, provider, input.playbackState.startUrlPlayback ?? startUrlPlayback)
    ...
    const midPlaybackReplacementDjProgramRequest = extractInlineDjModeReplacementRequest(userText, input.playbackState);
    if (midPlaybackReplacementDjProgramRequest && input.playbackState) {
      input.playbackState.pendingStationRequest = undefined;
      input.playbackState.pendingStationOriginalRequest = undefined;
      input.playbackState.pendingStationNeedsChoice = false;
      cancelDjProgramPreparations(input.playbackState);
      ...
      const prepared = await prepareDjProgramRequest({
        ...
        provider,
        llm,
        requestText: midPlaybackReplacementDjProgramRequest,
        buildContext: input.buildContext,
        writeStatus,
        playbackState: input.playbackState,
        synthesize,
    ...
    const currentStationDjVersionRequest = extractCurrentStationDjVersionRequest(userText, input.playbackState)
      ?? (isMidStationDjModeRequest(userText, input.playbackState) ? input.playbackState?.station?.request ?? null : null);
    if (currentStationDjVersionRequest && input.playbackState) {
      input.playbackState.pendingDjVersionRequest = currentStationDjVersionRequest;
      const response = formatMidStationDjModeResponse(config);
      store.addMessage(sessionId, "pockedio", response);
      writeOutput(response);
      return { sessionId, intent: { type: "pending_station_dj_program", confidence: "high" }, response, shouldExit: false };
    }

    if (intent.type === "playback_status") {
      const response = formatPlaybackStatus(input.playbackState, config, now());
      store.addMessage(sessionId, "pockedio", response);
      writeOutput(response);
      return { sessionId, intent, response, shouldExit: false };
    }
    ...
    if (intent.type === "playback_request" || intent.type === "direct_playback_request") {
      const ambiguousGluedRequest = input.playbackState
        ? await prepareGluedSongOrStationSelection({
          playbackState: input.playbackState,
          provider,
          requestText: userText,
          writeStatus,
        ...
        : null;
      if (ambiguousGluedRequest) {
        store.addMessage(sessionId, "pockedio", ambiguousGluedRequest);
        writeOutput(ambiguousGluedRequest);
        return { sessionId, intent, response: ambiguousGluedRequest, shouldExit: false };
      }

      const inlineDjModeStationRequest = extractInlineDjModeStationRequest(userText);
      const explicitDjProgram = isExplicitDjProgramPlaybackRequest(userText) || Boolean(inlineDjModeStationRequest);
      ...
      if (explicitDjProgram && input.playbackState) {
        const prepared = await prepareDjProgramRequest({
          ...
          provider,
          llm,
          requestText: inlineDjModeStationRequest ?? userText,
          buildContext: input.buildContext,
          writeStatus,
          playbackState: input.playbackState,
          synthesize,
...
function formatInteractiveLivePromptArea(text = "", options: TuiRenderOptions = {}): string {
  return createInteractiveLivePromptLayout(text, text.length, options).area;
}

type InteractiveLivePromptLayout = {
  area: string;
  inputRows: number;
  cursorRow: number;
  cursorColumn: number;
...
function createInteractiveLivePromptLayout(text: string, cursor: number, options: TuiRenderOptions = {}): InteractiveLivePromptLayout {
  const width = Math.max(0, options.width ?? (options.color ? 32 : 0));
  const prompt = formatInteractiveLivePrompt(options);
  const promptWidth = 2;
  if (!options.color || width <= promptWidth) {
    return {
      area: `${prompt}${text}`,
      inputRows: 1,
      cursorRow: 0,
      cursorColumn: Math.max(0, promptWidth + promptDisplayWidth(text.slice(0, cursor)))
    };
  }

  const rows: Array<{ text: string; width: number }> = [{ text: prompt, width: promptWidth }];
  let consumed = 0;
  let cursorRow = 0;
  let cursorColumn = promptWidth;
  const setCursor = () => {
    cursorRow = rows.length - 1;
    cursorColumn = rows.at(-1)!.width;
  };
  ...
  for (const char of text) {
    ...
    const charWidth = promptDisplayWidth(char);
    let row = rows.at(-1)!;
    if (row.width > promptWidth && row.width + charWidth > width) {
      row = { text: "  ", width: promptWidth };
      rows.push(row);
    }
    row.text += char;
    row.width += charWidth;
  ...
  return {
    area: `${rule}\n${rows.map((row) => `${row.text}${" ".repeat(Math.max(0, width - row.width))}`).join("\n")}\n${rule}`,
  ...
  return new Promise((resolve, reject) => {
    ...
    const render = () => {
      if (hasRenderedPrompt) {
        clearInteractiveLivePromptBox(output, promptView);
      }
      const currentOptions = renderOptions();
      const layout = createInteractiveLivePromptLayout(promptView.line, promptView.cursor, currentOptions);
      output.write(layout.area);
      promptView.renderedRows = layout.inputRows;
      promptView.cursorRow = layout.cursorRow;
      hasRenderedPrompt = true;
      moveCursor(output, 0, -(layout.inputRows - layout.cursorRow));
      cursorTo(output, layout.cursorColumn);
    };
    const onResize = () => {
      render();
    };
...
export function clearInteractiveSubmittedInputEcho(output: TerminalOutput, promptView?: ReadlinePromptView): void {
  clearInteractiveLivePromptBox(output, promptView);
}

function clearInteractiveLivePromptBox(output: TerminalOutput, promptView?: ReadlinePromptView): void {
  if (!output.isTTY) {
    return;
  }
  if (promptView) {
    const inputRows = Math.max(1, promptView.renderedRows ?? 1);
    const cursorRow = Math.max(0, Math.min(inputRows - 1, promptView.cursorRow ?? 0));
    const totalRows = inputRows + 2;
    moveCursor(output, 0, -(cursorRow + 1));
    ...
    moveCursor(output, 0, -(totalRows - 1));
    cursorTo(output, 0);
    return;
  }
  clearLine(output, 0);
...
export function createPromptSafeOutputWriter(
  ...
  return (text) => {
    ...
    clearInteractiveLivePromptBox(output, rl);
    output.write(`${text}\n`);
    const options = { color: true, width: output.columns };
    const layout = createInteractiveLivePromptLayout(line, cursor, options);
    output.write(layout.area);
    rl.renderedRows = layout.inputRows;
    rl.cursorRow = layout.cursorRow;
    moveCursor(output, 0, -(layout.inputRows - layout.cursorRow));
    cursorTo(output, layout.cursorColumn);
  };
}
...
export function formatInitialInteractiveTurnStatus(input: string, playbackState?: InteractivePlaybackState): string | undefined {
  ...
  if (extractInlineDjModeStationRequest(userText)) {
    return "Preparing DJ program...";
  }
...
async function handleSingleTrackRequest(input: {
  ...
  if (!parsed.artist && input.playbackState && shouldAskSongOrStation(input.requestText, parsed.keyword, candidates)) {
    input.playbackState.pendingSongOrStationSelection = {
    ...
    input.playbackState.pendingSingleTrackSelection = undefined;
    return { response: formatSongOrStationChoices(input.playbackState.pendingSongOrStationSelection) };
  }
...
async function prepareGluedSongOrStationSelection(input: {
  playbackState: InteractivePlaybackState;
  provider: MusicProvider;
  requestText: string;
  writeStatus: StatusWriter;
  ...
  const parsed = parseGluedSongOrStationRequest(input.requestText);
  if (!parsed) {
    return null;
  }
  ...
  if (candidates.length === 0 || !hasCloseTitleMatch(parsed.query, candidates)) {
    return null;
  }
  ...
  input.playbackState.pendingSingleTrackSelection = undefined;
  return formatSongOrStationChoices(input.playbackState.pendingSongOrStationSelection);
}
...
async function handleSingleTrackStart(input: {
  ...
  if (input.playbackState) {
    ...
    nowPlaying = await withStatus(input.writeStatus, "Starting playback...", () => startTrackAt(input.playbackState!, input.config, input.store, input.provider, 0, input.startUrlPlayback, input.now), input.signal);
...
function parseGluedSongOrStationRequest(text: string): { query: string } | null {
  const match = text.trim().match(/^(?:please\s+)?(?:play|queue|start)(\S.+)$/i);
  const query = match?.[1]?.replace(/[.!?]+$/g, "").trim();
  if (!query || query.length > 40 || !containsCjkText(query)) {
    return null;
  }
  return { query };
}

function shouldAskSongOrStation(requestText: string, query: string, candidates: StationTrack[]): boolean {
  return /^(?:please\s+)?(?:play|queue|start)\s+\S+/i.test(requestText.trim())
    && containsCjkText(query)
    && query.length <= 40
    && hasCloseTitleMatch(query, candidates);
}

function hasCloseTitleMatch(query: string, candidates: StationTrack[]): boolean {
  const normalizedQuery = normalizeTitleMatchText(query);
  return candidates.some((candidate) => {
    const title = normalizeTitleMatchText(candidate.title);
...
function formatSongOrStationChoices(selection: PendingSongOrStationSelection): string {
  return formatSongOrStationChoiceSurface(selection, 0);
}

function formatSongOrStationChoiceSurface(selection: PendingSongOrStationSelection, selectedIndex: number): string {
  return [
    `How should I use "${selection.query}"?`,
    renderTuiChoiceList([
      { label: "Play a song match", meta: "choose from close matches" },
      { label: "Build a 5-song station", meta: "use it as a vibe" }
    ], selectedIndex, {
      color: Boolean(defaultOutput.isTTY),
      width: defaultOutput.columns
    }),
...
function isPendingSongOrStationCancel(playbackState: InteractivePlaybackState | undefined, text: string): boolean {
  return Boolean(playbackState?.pendingSongOrStationSelection) && /^(b|back|cancel|esc|escape|q)$/i.test(text.trim());
}

function getPendingSongOrStationChoice(
  playbackState: InteractivePlaybackState | undefined,
  text: string
): "song" | "station" | undefined {
  if (!playbackState?.pendingSongOrStationSelection) {
    return undefined;
  }
  const normalized = text.trim().toLowerCase();
  if (normalized === "1" || normalized === "song" || normalized === "play") {
    return "song";
  }
  if (normalized === "2" || normalized === "station" || normalized === "vibe") {
    return "station";
  }
  return undefined;
}

function formatQueueToneChangeChoices(selection: PendingQueueToneChangeSelection): string {
  return formatQueueToneChangeChoiceSurface(selection, 0);
}

function formatQueueToneChangeChoiceSurface(selection: PendingQueueToneChangeSelection, selectedIndex: number): string {
  const isFinalTrackOnly = selection.remainingCount === 1;
  return [
    isFinalTrackOnly ? "Only one track is left in this station." : "Change the current station?",
    "",
    renderTuiChoiceList([
      isFinalTrackOnly
        ? { label: "Replace final track", meta: "replace only the next slot" }
        : { label: "Reshape remaining queue", meta: "replace tracks after the current one" },
      { label: "Build new station", meta: "start a fresh station" },
      { label: "Keep current queue", meta: "leave playback unchanged" }
    ], selectedIndex, {
      color: Boolean(defaultOutput.isTTY),
      width: defaultOutput.columns
    }),
...
function isPendingQueueToneChangeCancel(playbackState: InteractivePlaybackState | undefined, text: string): boolean {
  return Boolean(playbackState?.pendingQueueToneChangeSelection) && /^(b|back|cancel|esc|escape|q)$/i.test(text.trim());
}

function getPendingQueueToneChangeChoice(
  playbackState: InteractivePlaybackState | undefined,
  text: string
): QueueToneChangeChoice | undefined {
  if (!playbackState?.pendingQueueToneChangeSelection) {
    return undefined;
  }
  const normalized = text.trim().toLowerCase();
  if (!normalized || normalized === "1" || normalized === "reshape" || normalized === "remaining" || normalized === "final" || normalized === "replace final" || normalized === "soften" || normalized === "soften it" || normalized === "soften final" || normalized === "last one" || normalized === "just the last one") {
    return "reshape";
  }
  if (normalized === "2" || normalized === "new" || normalized === "new station" || normalized === "build new station" || normalized === "start over") {
    return "new_station";
  }
  if (normalized === "3" || normalized === "keep" || normalized === "keep current" || normalized === "keep current queue" || normalized === "never mind" || normalized === "no") {
    return "keep";
  }
  return undefined;
}
  ...
  if (input.playbackState) {
    ...
    input.playbackState.djProgram = input.djProgramIntro && input.synthesize
      ? {
	      ...
	      : undefined;
    stopActivePlayback(input.playbackState, input.store);
    nowPlaying = await withStatus(input.writeStatus, "Starting playback...", () => startTrackAt(input.playbackState!, input.config, input.store, input.provider, 0, input.startUrlPlayback, input.now, {
...
async function prepareCurrentStationDjProgram(input: {
  config: PockedioConfig;
  sessionId: string;
  store: MemoryStore;
  llm: LlmClient;
  ...
  if (!station || !storedTracks || storedTracks.length === 0) {
    return {
      station: station ?? { request: input.requestText, source: "fallback", tracks: [] },
      response: "No current station is available to convert into DJ mode."
    };
  }
...
async function generateStationIntroResponse(input: {
  ...
  const prompt = [
    ...
    `Pockedio is the app name, not your DJ name. If you name yourself, say you are ${displayName}, never Pockedio.`,
...
async function generateMusicRecommendationResponse(input: {
  ...
  const prompt = [
    ...
    `Diary summary: ${input.context?.diary?.summary ?? "not available"}`,
    `Diary listening hint: ${input.context?.diary?.listeningHint ?? "not available"}`,
    formatConversationTasteContext(input.config, input.userText),
    formatConversationPlaybackContext(input.playbackState),
    `User: ${input.userText}`
  ].join("\n");
  const result = await input.llm.generateText(prompt, { signal: input.signal });
  if (isUsableGeneratedText(input.config, result)) {
    const text = sanitizeGeneratedDjCopyForLocalTime({
      text: result.value,
    ...
    if (text) {
      return appendPendingStationChoicePrompt(formatDjNoteSurface(text, input.config));
    }
    return appendPendingStationChoicePrompt(formatDjNoteSurface(formatTimeSafeRecommendationFallback(input.config), input.config));
  }

  const diaryBasedFallback = formatDiaryBasedRecommendationFallback(input.context, input.userText);
  if (diaryBasedFallback) {
    return appendPendingStationChoicePrompt(formatDjNoteSurface(diaryBasedFallback, input.config));
  }

  const tasteBasedFallback = formatTasteBasedRecommendationFallback(input.config, input.userText);
  if (tasteBasedFallback) {
    return appendPendingStationChoicePrompt(formatDjNoteSurface(tasteBasedFallback, input.config));
  }
...
function isMidStationDjModeRequest(text: string, playbackState: InteractivePlaybackState | undefined): boolean {
  ...
  return Boolean(playbackState?.currentTrackId)
    ...
    && (normalized === "dj" || normalized === "dj mode" || /\b(dj mode|dj program|dj version|radio version|spoken version)\b/.test(normalized));
}
...
function extractCurrentStationDjVersionRequest(text: string, playbackState: InteractivePlaybackState | undefined): string | null {
  const normalized = text.trim().toLowerCase();
  if (!playbackState?.currentTrackId || playbackState.pendingStationRequest || !playbackState.station?.request) {
    return null;
  }
  return /\b(new|another|fresh|make this|turn this into|as a)\b.*\b(dj program|dj version|radio version|spoken version)\b/.test(normalized)
    ? playbackState.station.request
    : null;
}

function extractInlineDjModeStationRequest(text: string): string | null {
  if (!/\b(dj mode|dj program|dj version|radio show|radio version|spoken version)\b/i.test(text)) {
    return null;
  }
  ...
  const request = text
    ...
    .replace(/^[,\s]+|[,\s.!?]+$/g, "")
    .trim();
  return /\b(jazz|ambient|classical|piano|lofi|lo-fi|rock|pop|edm|folk|hip hop|r&b|instrumental|chill|relaxing|focus|soft|morning|night|rainy|music|songs|tracks|set)\b/i.test(request)
    ? request
    : null;
}
...
function formatMidStationDjModeResponse(config: PockedioConfig): string {
  return [
    `I can prepare a spoken DJ version of this same station with ${formatRuntimeDjDisplayName(config)}.`,
    "",
...
async function handlePendingStationFollowup(input: {
  ...
  return appendPendingStationChoicePrompt(formatDjNoteSurface(`${formatPendingStationRefinementFallback(input.userText)}\n\nPlay this version?`, input.config));
}
...
function formatConversationPrompt(
  ...
  return [
    ...
    "For direct questions about the listener's taste, base the answer on Taste context and describe the style or listening pattern when taste context exists.",
...
function sanitizeGeneratedDjCopyForLocalTime(input: {
  text: string;
  userText: string;
  context: Partial<PockedioContext> | undefined;
  config?: PockedioConfig;
}): string | null {
  const sanitized = sanitizeGeneratedDjCopy(input.text.trim(), input.config);
  return hasCurrentTimeClaimConflict(sanitized, input.userText, input.context)
    ? null
    : alignGeneratedTextToLocalDaypart(sanitized, input.userText, input.context);
}

function hasCurrentTimeClaimConflict(
  text: string,
  userText: string,
  context: Partial<PockedioContext> | undefined
): boolean {
  ...
  const normalized = text.toLowerCase();
  return localDaypart !== "morning" && morningCurrentTimeClaimPattern.test(normalized);
}

const morningCurrentTimeClaimPattern = /\b(good morning|start(?:ing)?\s+(?:your|the|my)?\s*morning|start(?:ing)?\s+(?:your|the|my)?\s*day|day\s+(?:begins|starts)|sunrise|fresh start)\b/i;

function formatTimeSafeRecommendationFallback(config: PockedioConfig): string {
  return [
    `I’d keep this one time-neutral and shape a five-track set around ${formatRuntimeDjDisplayName(config)}’s current read of your taste and mood.`,
    "",
    "Want me to build that station?"
  ].join("\n");
}
...
function rewriteDjSelfIdentification(text: string, config?: PockedioConfig): string {
  ...
  return text
    .replace(/\b(I['’]m|I am)\s+Pockedio\b/gi, (_match, prefix: string) => `${prefix} ${displayName}`)
    .replace(/\b(This is)\s+Pockedio\b/gi, (_match, prefix: string) => `${prefix} ${displayName}`)
    .replace(/\bPockedio\s+here\b/gi, `${displayName} here`);
}
...
function formatConversationTasteContext(config: PockedioConfig | undefined, userText: string): string {
  if (!shouldUseTasteContext(userText)) {
    return "";
  }

  const taste = readTasteMarkdown(config);
  if (!taste || !config || !hasTasteSignals(config.paths.taste)) {
...
function isTasteBasedListeningRequest(text: string): boolean {
  const normalized = text.trim().toLowerCase();
  return /\b(my taste|my music taste|music taste|listening taste|taste\.md|based on .*taste)\b/.test(normalized)
    && /\b(suggest|recommend|what should i listen|listen to|music|songs|tracks|station|playlist|play)\b/.test(normalized);
}

function isDiaryBasedListeningRequest(text: string): boolean {
  return /\b(mood|context|day|moment|vibe)\b/i.test(text);
}

function shouldUseTasteContext(text: string): boolean {
  return isTasteInsightQuestion(text) || isTasteBasedListeningRequest(text);
}
...
function formatConversationFallback(userText: string, playbackState: InteractivePlaybackState | undefined, config?: PockedioConfig): string {
  ...
  const tasteInsight = formatTasteInsightFallback(config, userText);
  if (tasteInsight) {
    return tasteInsight;
  }
...
function formatTasteInsightFallback(config: PockedioConfig | undefined, userText: string): string | null {
  if (!isTasteInsightQuestion(userText)) {
    return null;
  }

  const summary = buildTasteStyleSummary(config);
  if (!summary) {
    return "I do not have enough saved taste data yet. Import a playlist or react during playback, then I can describe your listening style from taste.md.";
  }

  return `Based on taste.md, ${summary.styleSentence}.${summary.anchorSentence ? ` ${summary.anchorSentence}` : ""}`;
}

function formatTasteBasedRecommendationFallback(config: PockedioConfig, userText: string): string | null {
  if (!isTasteBasedListeningRequest(userText)) {
    return null;
  }
  ...
  if (!summary) {
    return [
      "I do not have enough saved taste data yet, so I would start with a gentle discovery station.",
      "",
      "Want me to build that station?"
    ].join("\n");
  }
  ...
  return [
    `Based on taste.md, ${summary.styleSentence}.`,
    "I would start with a warm, low-pressure station that keeps those anchors in view instead of jumping to a random genre.",
    "",
    "Want me to build that station?"
  ].join("\n");
}

function formatDiaryBasedRecommendationFallback(
  context: Partial<PockedioContext> | undefined,
  userText: string
): string | null {
  if (!isDiaryBasedListeningRequest(userText)) {
    return null;
  }

  const hint = context?.diary?.listeningHint?.trim();
  if (!hint || /^diary listening hint unavailable/i.test(hint)) {
    return null;
  }

  return [
    `I’m using the diary signal from your recent entry: ${hint}`,
    "I’d build a five-track station around that mood.",
    "",
    "Want me to build that station?"
  ].join("\n");
}

function buildTasteStyleSummary(config: PockedioConfig | undefined): { styleSentence: string; anchorSentence: string } | null {
  const taste = readTasteMarkdown(config);
  if (!taste || !config || !hasTasteSignals(config.paths.taste)) {
    return null;
  }

  const style = inferTasteStyleDescriptors(taste);
  const anchors = extractTasteAnchors(taste);
  const styleSentence = style.length > 0
    ? `your saved taste leans toward ${formatNaturalList(style)}`
    : "your saved taste is anchored by the artists and tracks in taste.md";
  const anchorSentence = anchors.length > 0
    ? `Anchors I see: ${anchors.join("; ")}.`
    : "";
  return { styleSentence, anchorSentence };
}

function inferTasteStyleDescriptors(markdown: string): string[] {
  const normalized = markdown.toLowerCase();
  const descriptors: Array<[string, RegExp]> = [
    ["spacious instrumental music", /\b(instrumental|sakamoto|r[a-z]*yuichi|piano|composer|classical)\b/],
    ["ambient texture", /\b(ambient|brian eno|eno|ascent|drone|soundscape)\b/],
    ["cinematic or reflective pieces", /\b(cinematic|film|score|soundtrack|christmas mr\.? lawrence|reflective)\b/],
    ["soft jazz or bossa colors", /\b(jazz|bossa|lisa ono|小野リサ)\b/],
    ["warm soul and R&B", /\b(soul|r&b|rnb|leon bridges|mick)\b/],
    ["low-pressure focus listening", /\b(focus|work|reading|study|deep work|low-pressure|patient)\b/],
    ["downtempo electronic mood", /\b(downtempo|electronic|lo-fi|lofi|trip hop)\b/]
  ];
  return descriptors
...
function extractTasteAnchors(markdown: string): string[] {
  const explicitAnchors = markdown
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.startsWith("- "))
    .map((line) => line.replace(/^- /, "").trim())
    .filter((line) => line && !/^no\s+/i.test(line))
    .filter((line) => !/^last import:/i.test(line))
    .filter((line) => !/^imported tracks:\s*\d+/i.test(line))
    .filter((line) => !/^generated from/i.test(line))
    .filter((line) => !/^add durable/i.test(line))
    .filter((line) => !/^add negative/i.test(line))
    .filter((line) => !/^add situational/i.test(line))
    .map((line) => line.replace(/\s+\([^)]*\)$/u, "").trim())
    .filter(Boolean);

  return [...new Set(explicitAnchors)].slice(0, 4);
}
...
function formatPendingStationRefinementFallback(userText: string): string {
  const normalized = userText
    .trim()
    .replace(/[.。！？!?]+$/u, "")
    .replace(/\brandom musics\b/gi, "random music")
    .replace(/\bbased on my mood\b/gi, "based on your mood")
    .replace(/\bbased on my taste\b/gi, "based on your taste");
  if (/\bbased on your mood\b/i.test(normalized)) {
    return "Got it. I’ll shape this around your mood.";
  }
  if (/\bbased on your taste\b/i.test(normalized)) {
    return "Got it. I’ll shape this around your taste.";
  }
  return `Got it. I’ll shape it around: ${normalized}.`;
}
...
function isActiveStationChangeRequest(userText: string, playbackState: InteractivePlaybackState | undefined): boolean {
  if (!playbackState?.currentTrackId || !playbackState.station) {
    return false;
  }
  const text = userText.trim().toLowerCase();
  if (isExplicitDjProgramPlaybackRequest(text) || /\b(dj mode|dj program|dj version|radio show|radio version|spoken version)\b/.test(text) || isReplacementStationRequestLikeText(text)) {
    return false;
  }
  return /^(change|shift|switch)\s+(?:to|into|toward|towards)\s+.+/.test(text)
    || /\b(change|shift|switch)\b.+\b(vibe|mood|tone|style|genre|queue|station|song|songs|music)\b/.test(text);
}

function isReplacementStationRequestLikeText(text: string): boolean {
...
export async function runInteractiveSession(config: PockedioConfig = loadConfig()): Promise<InteractiveSessionOutcome> {
  ...
  runMigrations(config);
  const store = new MemoryStore(config);
  store.cleanupExpiredDjAudioCache();
  store.cleanupOrphanedDjAudioFiles(config.paths.djAudioDir);
    ...
    while (true) {
      ...
      try {
        ...
        if (!usesSingleTrackInlinePicker && !usesSongOrStationInlinePicker && !usesQueueToneChangeInlinePicker && !usesFavoriteListInlinePicker) {
          clearInteractiveSubmittedInputEcho(defaultOutput, livePromptView);
        }
        if (submittedTurn) {
...
function promptPendingSongOrStationSelection(selection: PendingSongOrStationSelection): Promise<string> {
  return new Promise((resolve) => {
    let selectedIndex = 0;
    const input = defaultInput;
    const output = defaultOutput;
    ...
    const onKeypress = (_value: string, key: Key) => {
      ...
      const numericIndex = Number(key.name) - 1;
      if (Number.isInteger(numericIndex) && numericIndex >= 0 && numericIndex < 2) {
        cleanup(String(numericIndex + 1));
        return;
      }
      if (key.name === "b" || key.name === "q" || (key.ctrl && key.name === "c")) {
...
function promptPendingQueueToneChangeSelection(selection: PendingQueueToneChangeSelection): Promise<string> {
  return new Promise((resolve) => {
    let selectedIndex = 0;
    const input = defaultInput;
    const output = defaultOutput;
...
async function reshapeRemainingQueueForFeedback(input: {
  ...
  if (input.action === "change_vibe") {
    const selection: PendingQueueToneChangeSelection = {
      action: input.action,
      feedbackText: input.feedbackText,
      replacementRequest: buildFeedbackReplacementStationRequest(station.request, input.currentTrack, input.action, input.feedbackText),
      remainingCount
    };
    input.playbackState.pendingQueueToneChangeSelection = selection;
    return formatQueueToneChangeChoices(selection);
  }

  return applyRemainingQueueReshapeForFeedback(input);
}

async function applyRemainingQueueReshapeForFeedback(input: {
  config: PockedioConfig;
  sessionId: string;
  store: MemoryStore;
  provider: MusicProvider;
  ...
  const request = buildFeedbackReshapeRequest(station.request, input.currentTrack, input.action, input.feedbackText);
...
function buildFeedbackReshapeRequest(baseRequest: string, currentTrack: StationTrack, action: FeedbackAction, feedbackText = ""): string {
  ...
  const explicitFeedback = feedbackText.trim() ? ` Listener wording: ${feedbackText.trim()}.` : "";
  return `${baseRequest}. Feedback for remaining queue: ${direction}.${explicitFeedback}`;
}

function buildFeedbackReplacementStationRequest(baseRequest: string, currentTrack: StationTrack, action: FeedbackAction, feedbackText: string): string {
  const reshapeRequest = buildFeedbackReshapeRequest(baseRequest, currentTrack, action, feedbackText);
  return `${reshapeRequest} Build a fresh five-track station instead of only replacing the final slot.`;
}
...
async function generateStationDjProgramIntro(input: {
  ...
  const textResult = await input.llm.generateText([
    ...
    `Pockedio is the app name, not your DJ name. If you name yourself, say you are ${formatRuntimeDjDisplayName(input.config)}, never Pockedio.`,
  ...
  const generatedText = isUsableGeneratedText(input.config, textResult)
    ? sanitizeGeneratedDjCopyForLocalTime({
  ...
  const djAudio = shouldUseSpokenDjAudio({
    ...
    ? await input.synthesize(input.config, text, { signal: input.signal, timeoutMs: EXPLICIT_DJ_AUDIO_TIMEOUT_MS })
  ...
  const fileSizeBytes = getFileSize(djAudio.audioPath);
  const storedAudioPath = releasePlayedDjAudioPath(djAudio.audioPath, playback.ok);
  input.store.recordDjAudio(input.sessionId, "explicit", null, text, storedAudioPath, playback.ok ? "played" : "failed", {
    cacheExpiresAt: getExplicitDjAudioCacheExpiresAt(),
    latencyMs: djAudio.latencyMs,
    fileSizeBytes
  });
  ...
  return {
    ...
    rawText: text,
    audioPath: playback.ok ? undefined : storedAudioPath ?? undefined,
    latencyMs: djAudio.latencyMs,
    fileSizeBytes
  };
}
...
function releasePlayedDjAudioPath(audioPath: string, played: boolean, env: NodeJS.ProcessEnv = process.env): string | null {
  if (!played || shouldKeepDjAudioFiles(env)) {
    return audioPath;
  }
  try {
...
function shouldKeepDjAudioFiles(env: NodeJS.ProcessEnv = process.env): boolean {
  return /^(1|true|yes)$/i.test(env.POCKEDIO_KEEP_DJ_AUDIO?.trim() ?? "");
}
...
function stopActivePlayback(playbackState: InteractivePlaybackState, store: MemoryStore): void {
  ...
  invalidatePlaybackGeneration(playbackState);
  const activePlayback = playbackState.activePlayback;
  playbackState.activePlayback = undefined;
...
function invalidatePlaybackGeneration(playbackState: InteractivePlaybackState): void {
  playbackState.playbackGeneration = (playbackState.playbackGeneration ?? 0) + 1;
}

function isPlaybackGenerationSuperseded(
  playbackState: InteractivePlaybackState,
  expectedPlaybackGeneration: number | undefined
): boolean {
  return expectedPlaybackGeneration !== undefined
    && playbackState.playbackGeneration !== expectedPlaybackGeneration;
}
...
async function refreshTrackPlayableUrl(track: StationTrack, provider: MusicProvider): Promise<PlayableTrack> {
...
async function startTrackAt(
  ...
  if (isPlaybackGenerationSuperseded(playbackState, options.expectedPlaybackGeneration)) {
    return PLAYBACK_TRANSITION_SUPERSEDED;
  }
  const storedTracks = playbackState.storedTracks ?? [];
  ...
  const playbackGeneration = playbackState.playbackGeneration ?? 0;
  playbackState.playbackGeneration = playbackGeneration;
  ...
  handle.done.then((result) => {
    if (playbackState.activePlayback !== handle || playbackState.playbackGeneration !== playbackGeneration) {
      return;
    }
    ...
    if (!result.ok) {
      void handleActivePlaybackFailure(playbackState, config, provider, entry, result, nextIndex + 1, startUrlPlayback);
      return;
    }
    playbackState.consecutivePlaybackFailures = 0;
    void autoAdvancePlayback(playbackState, config, provider, entry.dbId, nextIndex + 1, startUrlPlayback, playbackGeneration);
  }).catch(() => undefined);
...
function formatPreparedDjIntroForPlayback(config: PockedioConfig, intro: GeneratedDjProgramIntro): string {
  return intro.audioPath ? formatDjTranscript(config, intro.rawText) : intro.text;
}
...
async function handleActivePlaybackFailure(
  ...
  try {
    ...
    const nextResponse = await startTrackAt(playbackState, config, store, provider, startIndex, startUrlPlayback, () => new Date(), {
      intro: preparedIntro,
      startDuckedIntroPlayback: playbackState.startDuckedIntroPlayback,
...
async function autoAdvancePlayback(
  ...
  const store = new MemoryStore(config);
  try {
    if (isPlaybackGenerationSuperseded(playbackState, expectedPlaybackGeneration)) {
      return;
    }
    ...
    if (response === PLAYBACK_TRANSITION_SUPERSEDED) {
      return;
    }
...
async function generateTrackDjProgramIntro(input: {
  ...
  const djAudio = await input.synthesize(input.config, text, { signal: input.signal, timeoutMs: EXPLICIT_DJ_AUDIO_TIMEOUT_MS });
  if (!djAudio.ok) {
    return { text: formatDjAudioFallbackText(text, djAudio.error), rawText: text };
  }
  return {
...
function recordDjProgramIntroPlayback(
  ...
  const storedAudioPath = releasePlayedDjAudioPath(intro.audioPath, playback?.ok === true);
  store.recordDjAudio(sessionId, "explicit", null, intro.rawText, storedAudioPath, playback?.ok ? "played" : "failed", {
    cacheExpiresAt: getExplicitDjAudioCacheExpiresAt(),
...
function formatPlaybackStatus(playbackState: InteractivePlaybackState | undefined, config: PockedioConfig, now: Date): string {
  if (!playbackState) {
  ...
  if (!storedTracks[currentIndex]?.track) {
    return "No station is playing.";
  }
  ...
  return renderPlaybackSurface({
    queue: storedTracks,
    currentIndex,
    currentStartedAt: playbackState.currentStartedAt,
    now,
